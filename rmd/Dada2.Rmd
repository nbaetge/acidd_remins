...
```{r}
path <- "~/GITHUB/acidd_remins/input/16s/combined_fastq"
...
plotQualityProfile(fnFs[1:39])

#144 B0 doesn't look like a good sample
```
...
```{r}
saveRDS(mergers, "~/GITHUB/acidd_remins/output/dada_merged.rds")
...
```{r}
taxa <- assignTaxonomy(seqtab.nochim, "~/GITHUB/acidd_remins/input/16s/Reference_Database/silva_nr_v138_train_set.fa", multithread = TRUE)
```
...
```{r}
saveRDS(t(seqtab.nochim), "~/GITHUB/acidd_remins/output/seqtab-nochimtaxa.rds")
saveRDS(taxa,"~/GITHUB/acidd_remins/output/taxa.rds")
